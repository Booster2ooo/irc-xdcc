  ...
  , ircXdcc = function ircXdcc(server, nick, opt) {
        return new Promise(function(moduleResolve, moduleReject) {
            try {
                var // will hold irc Client instance
                  ...
                  , _removePool = function _removePool(index) {
                        return new Promise(function(resolve, reject) {
                            ...
                            xdcc.cancel();
                            ircClient.emit(_eventsNames.xdcc_started, xdcc);
                            Promise
                ...
                ircClient.on(_eventsNames.irc_registered, handlers.registered);
                // bind handler to disconnection events
                ircClient.on(_eventsNames.irc_quit, handlers.disconnected);
                ircClient.on(_eventsNames.irc_kill, handlers.disconnected);
